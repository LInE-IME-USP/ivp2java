...
public class IVPMouseListener extends MouseAdapter {
  ...
  private void dropComponent (IVPContainer target, int dropY) {
    String origin = ((DataObject) Services.getModelMapping().get(holdingComponent)).getParentID();
    String destiny = target.getCodeComposite();
    DataObject holdingParent = ((DataObject) Services.getModelMapping().get(origin));
    ...
    JComponent holdingJComponent = (JComponent) Services.getViewMapping().get(holdingComponent);
    if (holdingJComponent.isAncestorOf(target)) {
      ...
      Services.getController().moveChild(holdingComponent, origin, target.getCodeComposite(), originContext, destinyContext, target.getDropIndex(dropY, holdingJComponent));
      }
    holdingComponent = "";
    isHolding = false;
    lastEnteredComponent = null;
    lastYOnLastEnteredComponent = 0;
    Services.getController().changeCursor(Cursor.DEFAULT_CURSOR);
    }
