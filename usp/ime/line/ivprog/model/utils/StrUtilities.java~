...
public class StrUtilities {
	public static String readFromURL(java.applet.Applet applet, String strURL) {
		...
		try { //
			...
			try { // se falhou, tente completar com endereço base do applet e
				  // completar com nome de arquivo
				endURL = new java.net.URL(applet.getCodeBase().toString() + "/" + strURL); // se
																						   // for
																						   // URL
	...
	public static String inputStream2String(java.io.InputStream inputStream) {
		try {
			...
			try {
				...
				System.err.println("Error: reading from: " + inputStream + ": " + exception); // throw
																							  // Trace.error(34,
