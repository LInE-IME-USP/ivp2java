...
public class Variable extends Expression {
	...
	public String toXML() {
		String str = "<dataobject class=\"variable\">" + "<id>" + getUniqueID() + "</id>" + "<name>" + variableName + "</name>" + "<type>"
		        + variableType + "</type>" + "<value>" + variableValue + "</value>" + "<referencelist>";
		for (int i = 0; i < variableReferenceList.size(); i++) {
			VariableReference varRef = (VariableReference) Services.getService().getModelMapping()
			        .get((String) variableReferenceList.get(i));
			str += varRef.toXML();
		}
