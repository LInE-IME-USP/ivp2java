...
public class Function extends CodeComposite {
   ...
   * @see IVPConstants
   * @param funcReturnVoid : the returnType to set
   */
  ...
  /**
   * Remove a local variable with the specified name from the list and return it.
   * @param id
   */
  ...
  //TODO DUIDA SE PRECISAREI DISSO OU NAO... VOU TENTAR NAO DEPENDER DISSO...
  // PRECISAREI CRIAR UM OUVIDOR PRA QUAIS FUNCOES EXISTEM.
  /**
  ...
  public String toXML () {
    ...
    for (int i = 0; i < v1.size(); i++) {
      Variable param = (Variable) Services.getModelMapping().get((String) v1.get(i));
      str += param.toXML();
      }
    str += "</parameterlist><variablelist>";
    for (int i = 0; i < v2.size(); i++) {
      Variable var = (Variable) Services.getModelMapping().get((String) v2.get(i));
      str += var.toXML();
      }
  ...
  public String toJavaString () {
    ...
    for (int i = 0; i < paramList.size(); i++) {
      Variable v = ((Variable) Services.getModelMapping().get(paramList.get(i)));
      str += v.toJavaString();
      }
    str += ") {\n";
    // -- converting :
    ...
    for (int i = 0; i < varList.size(); i++) {
      Variable v = ((Variable) Services.getModelMapping().get(varList.get(i)));
      str += v.toJavaString();
      }
    str += "\n";
    Vector children = getChildrenList();
    for (int i = 0; i < children.size(); i++) {
      str += ((CodeComponent) Services.getModelMapping().get(children.get(i))).toJavaString() + "\n";
      }
    str += "   } " + "\n";
    return str;
    }
