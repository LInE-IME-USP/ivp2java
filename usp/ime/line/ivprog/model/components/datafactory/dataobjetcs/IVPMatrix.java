...
public class IVPMatrix extends Collection {
  ...
  public String toXML () {
    String str = "<dataobject class=\"ivpmatrix\">" + "<id>" + getUniqueID() + "</id>" + "<collectionname>" + getCollectionName()
            + "</collectionname>" + "<collectiontype>" + getCollectionType() + "</collectiontype>" + "<numberoflines>" + nLines
            + "</numberoflines><numberofcolumns>" + nColumns + "</numberofcolumns><elements>";
    for (int i = 0; i < nLines; i++) {
      for (int j = 0; j < nColumns; j++) {
        Variable anElement = (Variable) Services.getModelMapping().get(elements[i][j]);
