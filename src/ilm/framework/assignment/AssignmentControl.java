...
public final class AssignmentControl implements IAssignment, IAssignmentOperator, IlmProtocol {
  ...
  private void initModuleList () {
    ...
    // This module loads:
    // + usp.ime.line.ivprog.modules.configuration.ConfigurationModule extends ilm.framework.modules.IlmModule: "_gui = new ConfigurationToolBar();" that loads
    // + usp.ime.line.ivprog.modules.configuration.ConfigurationToolBar extends ilm.framework.modules.IlmModuleToolbar: "frame = new ConfigurationGUI();"
    if (!_config.isApplet()) // avoid security violation when "applet" (avoiding "new ConfigurationGUI" that extends JFrame)
      addModule(new ConfigurationModule());
    if (usp.ime.line.ivprog.Ilm.DEBUG) System.out.println("ilm/framework/assignment/AssignmentControl.java: initModuleList(): _config.isApplet=" + _config.isApplet());
    }
  ...
  private Assignment createNewAssignment () {
    ...
    Assignment assignment = new Assignment("", initialState, initialState, null);
    addAssignmentToModules();
  ...
  /**
   * TODO
   * This method is not altered in the protocol. For now return a 'float' between 0 and 1 (0 => 100% wrong, 1 => 100% correct)
   */
  ...
  /**
   * TODO
   * This method is not altered in the protocol. For now call the file register.
   */
  public String getAnswer () {
    ...
    for (int i = 0; i < _assignmentList.size(); i++) {
      ...
      if (((Assignment) _assignmentList.get(i)).getExpectedAnswer() == null
          || ((Assignment) _assignmentList.get(i)).getExpectedAnswer().getList().size() < 1
