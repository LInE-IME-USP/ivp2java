...
public final class AssignmentControl implements IAssignment, IAssignmentOperator, IlmProtocol {
	...
	/**
	 * Este método ainda não foi alterado no protocolo. Por enquanto, devolve um
	 * float entre 0 e 1.
	 */
	...
	/**
	 * Este método ainda não foi alterado no protocolo. Por enquanto, ainda
	 * chama a gravação do arquivo.
	 */
	public String getAnswer() {
		...
		for (int i = 0; i < _assignmentList.size(); i++) {
			...
			if (((Assignment) _assignmentList.get(i)).getExpectedAnswer() == null
			        ...
			        || !((Assignment) _assignmentList.get(i)).getInitialState().equals(
			                ((Assignment) _assignmentList.get(i)).getCurrentState())) {
				assignmentContent = parser.getAssignmentModulesData(_converter, assignmentContent, _moduleList, i);
			}
			assignmentContentList.add(assignmentContent);
		}
