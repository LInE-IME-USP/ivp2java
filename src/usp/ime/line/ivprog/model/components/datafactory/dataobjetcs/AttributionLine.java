...
public class AttributionLine extends CodeComponent {
  ...
  /**
   * @param leftVariable : the leftVariable to set
   */
  ...
  /**
   * @param leftVariableType : the leftVariableType to set
   */
  ...
  public String toXML () {
    VariableReference varLeft = (VariableReference) Services.getModelMapping().get(leftVariable);
    Expression rightExp = (Expression) Services.getModelMapping().get(rightExpression);
    String str = "<dataobject class=\"attline\">" + "<id>" + getUniqueID() + "</id>" + "<variabletype>" + leftVariableType
            + "</variabletype>" + "<left>" + varLeft.toXML() + "</left>" + "<right>" + rightExp.toXML() + "</right>" + "</dataobject>";
    return str;
    }
