...
public class DynamicFlowLayout extends java.awt.FlowLayout {
	...
	private void moveComponents(java.awt.Container target, int x, int y,
			int width, int height, int rowStart, int rowEnd, boolean ltr) {
		synchronized (target.getTreeLock()) {
			switch (getAlignment()) {
			case LEFT:
				x += ltr ? 0 : width;
	...
	public java.awt.Dimension preferredLayoutSize(java.awt.Container target) {
		...
		if (anchorComponent == null) {
			...
			maxwidth = anchorComponent.getWidth()
					- (insets.left + insets.right + hgap * 2) - anchorConstant;
		}
		int nmembers = target.getComponentCount();
		int x = 0, y = insets.top + vgap;
		int rowh = 0;
