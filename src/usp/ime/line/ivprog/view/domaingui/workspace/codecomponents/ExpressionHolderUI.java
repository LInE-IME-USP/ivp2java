...
public class ExpressionHolderUI extends JPanel implements IExpressionListener {
	...
	private void addStringOperations(JPopupMenu menu) {
		Action changeToConcatenation = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				Services.getService()
				        .getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_CONCAT, (short) -1, operationContext);
	...
	private void addBooleanOperators(JPopupMenu menu) {
		Action changeToAND = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_AND, (short) -1, operationContext);
		...
		Action changeToOR = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_OR, (short) -1, operationContext);
	...
	private void addArithmeticOperations(JPopupMenu menu) {
		Action createAddition = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_ADDITION, (short) -1, forContext);
				} else {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_ADDITION, (short) -1,
					                operationContext);
				}
			}
		};
		...
		Action createSubtraction = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_SUBTRACTION, (short) -1,
					                forContext);
				} else {
					Services.getService()
					        .getController()
		...
		createSubtraction
		        .putValue(Action.SHORT_DESCRIPTION, ResourceBundleIVP.getString("ExpressionHolderUI.action.createSubtraction.tip"));
		...
		Action createMultiplication = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_MULTIPLICATION, (short) -1,
					                forContext);
				} else {
					Services.getService()
					        .getController()
		...
		createMultiplication.putValue(Action.SHORT_DESCRIPTION,
		        ResourceBundleIVP.getString("ExpressionHolderUI.action.createMultiplication.tip"));
		...
		Action createDivision = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_DIVISION, (short) -1, forContext);
				} else {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_DIVISION, (short) -1,
					                operationContext);
				}
			}
		};
		...
		Action createIntDiv = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_INTDIV, (short) -1, forContext);
				} else {
					Services.getService()
					        .getController()
					        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_INTDIV, (short) -1,
					                operationContext);
				}
			}
		};
	...
	private void addComparison(JPopupMenu menu) {
		Action changeToLEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_LEQ, (short) -1, operationContext);
			}
		};
		...
		Action changeToLES = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_LES, (short) -1, operationContext);
			}
		};
		...
		Action changeToEQU = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_EQU, (short) -1, operationContext);
			}
		};
		...
		Action changeToNEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_NEQ, (short) -1, operationContext);
			}
		};
		...
		Action changeToGEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_GEQ, (short) -1, operationContext);
			}
		};
		...
		Action changeToGRE = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(expressionID, parentModelID, Expression.EXPRESSION_OPERATION_GRE, (short) -1, operationContext);
			}
		};
	...
	private void initChooseContentMenu() {
		...
		Action variableHasBeenChosen = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression("", parentModelID, Expression.EXPRESSION_VARIABLE, holdingType, forContext);
				} else {
					Services.getService().getController()
					        .createExpression("", parentModelID, Expression.EXPRESSION_VARIABLE, holdingType, operationContext);
				}
			}
		};
	...
	private void initChangeContentBtn() {
		...
		action.putValue(Action.SMALL_ICON,
		        new ImageIcon(IVPVariablePanel.class.getResource("/usp/ime/line/resources/icons/operations.png")));
	...
	private void expressionCreatedNormal(String holder, String id, String context) {
		if (holder.equals(parentModelID) && operationContext.equals(context)) {
			...
			if (Services.getService().getViewMapping().get(parentModelID) instanceof CodeBaseUI) { // nessa
																								   // atualização
																								   // tenho
																								   // que
																								   // olhar
	...
	private void expressionCreatedFor(String holder, String id, String context) {
		if (holder.equals(parentModelID) && forContext.equals(context)) {
			...
			if (Services.getService().getViewMapping().get(parentModelID) instanceof CodeBaseUI) { // nessa
				...
				if (lastExp != null && !"".equals(lastExp)) {
					Services.getService().getController()
					        .updateParent(parentModelID, ((IDomainObjectUI) lastExp).getModelID(), id, forContext);
				} else {
